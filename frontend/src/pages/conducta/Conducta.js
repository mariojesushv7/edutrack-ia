...
const niveles = [
    { valor: 'excelente', color: '#34d399', bg: 'rgba(52,211,153,0.1)', border: 'rgba(52,211,153,0.25)', label: 'Excelente' },
    { valor: 'bueno', color: '#60a5fa', bg: 'rgba(96,165,250,0.1)', border: 'rgba(96,165,250,0.25)', label: 'Bueno' },
    { valor: 'regular', color: '#fbbf24', bg: 'rgba(251,191,36,0.1)', border: 'rgba(251,191,36,0.25)', label: 'Regular' },
    { valor: 'malo', color: '#f87171', bg: 'rgba(248,113,113,0.1)', border: 'rgba(248,113,113,0.25)', label: 'Malo' },
];
...
const Conducta = () => {
    ...
    const seleccionarEstudiante = async (e) => {
        ...
        try {
            const r = await api.get(`/conducta/estudiante/${e.id}`);
            setHistorial(r.data);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setExito(form.nivel === 'malo' || form.nivel === 'regular' ? 'Conducta registrada — padre notificado' : 'Conducta registrada');
            setMostrarFormulario(false);
    ...
    const getNivel = (v) => niveles.find(n => n.valor === v);

    const resumen = () => {
        const c = { excelente: 0, bueno: 0, regular: 0, malo: 0 };
        historial.forEach(h => { if (c[h.nivel] !== undefined) c[h.nivel]++; });
        return c;
    };
    const res = resumen();
            ...
            <div style={estilos.inner}>
                <div style={estilos.navbar}>
                    <div style={estilos.navIzq}>
                        ...
                        <div>
                            <div style={estilos.navTituloTexto}>Conducta</div>
                            <div style={estilos.navSubtitulo}>Registro de comportamiento</div>
                        </div>
                    </div>
                </div>
                                ...
                                <>
                                    ...
                                    {historial.length > 0 && (
                                        <div style={estilos.resumenBox}>
                                            {niveles.map(n => (
                                                <div key={n.valor} style={{ ...estilos.resumenItem, backgroundColor: n.bg, border: `1px solid ${n.border}` }}>
                                                    <div style={{ ...estilos.resumenNum, color: n.color }}>{res[n.valor]}</div>
                                                    <div style={{ ...estilos.resumenLabel, color: n.color }}>{n.label}</div>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                        ...
                                        <div style={estilos.formulario}>
                                            <form onSubmit={handleSubmit}>
                                                <div style={estilos.nivelesGrid}>
                                                    {niveles.map(n => (
                                                        <div key={n.valor} onClick={() => setForm({ ...form, nivel: n.valor })} style={{
                                                            ...estilos.nivelOpcion,
                                                            backgroundColor: form.nivel === n.valor ? n.bg : 'rgba(255,255,255,0.03)',
                                                            border: `1px solid ${form.nivel === n.valor ? n.border : 'rgba(255,255,255,0.07)'}`,
                                                            color: form.nivel === n.valor ? n.color : 'rgba(255,255,255,0.35)',
                                                        }}>
                                                            <div style={{ fontWeight: '700', fontSize: '13px' }}>{n.label}</div>
                                                        </div>
                                                    ))}
                                                </div>
                                                <div style={{ marginTop: '12px' }}>
                                                    <label style={estilos.label}>Descripción (opcional)</label>
                                                    <textarea value={form.descripcion} onChange={(e) => setForm({ ...form, descripcion: e.target.value })} style={{ ...estilos.input, height: '70px', resize: 'vertical' }} placeholder="Describe el comportamiento..." />
                                                </div>
                                    ...
                                    <div style={estilos.listaHistorial}>
                                        {historial.length === 0 ? (
                                            ...
                                            return (
                                                <div key={h.id} style={{ ...estilos.historialFila, borderLeft: `3px solid ${n?.color}` }}>
                                                    <div style={{ ...estilos.nivelBadge, backgroundColor: n?.bg, color: n?.color, border: `1px solid ${n?.border}` }}>
                                                        {n?.label}
                                                    </div>
                                                    <div style={estilos.historialInfo}>
                                                        <div style={estilos.historialDesc}>{h.descripcion || 'Sin descripción'}</div>
                                                        <div style={estilos.historialMeta}>{new Date(h.fecha).toLocaleDateString()} · {h.docente_nombre}</div>
                                                    </div>
                                                </div>
                                            );
                                        })}
...
const estilos = {
    ...
    resumenBox: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', padding: '16px 24px', borderBottom: '1px solid rgba(255,255,255,0.05)' },
    resumenItem: { textAlign: 'center', padding: '12px', borderRadius: '10px' },
    resumenNum: { fontSize: '22px', fontWeight: '800' },
    resumenLabel: { fontSize: '11px', fontWeight: '600', marginTop: '2px' },
    formulario: { padding: '16px 24px', borderBottom: '1px solid rgba(255,255,255,0.05)', backgroundColor: 'rgba(255,255,255,0.02)' },
    nivelesGrid: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' },
    nivelOpcion: { padding: '12px', borderRadius: '8px', cursor: 'pointer', textAlign: 'center', transition: 'all 0.2s' },
    label: { fontSize: '10px', color: 'rgba(255,255,255,0.35)', marginBottom: '6px', fontWeight: '600', letterSpacing: '0.5px', textTransform: 'uppercase', display: 'block' },
    input: { width: '100%', padding: '9px 12px', backgroundColor: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '7px', fontSize: '13px', color: 'white', outline: 'none', boxSizing: 'border-box' },
    ...
    listaHistorial: { display: 'flex', flexDirection: 'column' },
    historialFila: { display: 'flex', alignItems: 'flex-start', gap: '14px', padding: '14px 24px', borderBottom: '1px solid rgba(255,255,255,0.04)' },
    nivelBadge: { padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: '700', whiteSpace: 'nowrap' },
    historialInfo: { flex: 1 },
    historialDesc: { color: 'rgba(255,255,255,0.7)', fontSize: '13px', lineHeight: '1.4' },
    historialMeta: { color: 'rgba(255,255,255,0.25)', fontSize: '11px', marginTop: '4px' },
